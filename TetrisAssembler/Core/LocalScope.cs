        ...
        public void Set(ScopeType scopeType, string name, IValue value) {
            if (scopeType == ScopeType.Local) {
                ...
                if (document.Constants.Get(name) != null)
                    throw new(string.Format("Can't set '{0}' a constant value with this name already exists", name));
                ...
                switch (scopeType) {
                    ...
                    default: throw new(string.Format("Can't set '{0}' of a unknown scope", name));
                }
            }
        }
    }
