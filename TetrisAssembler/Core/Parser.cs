...
namespace TetrisAssembler.Core {
    public class Parser : IDisposable {
        private static readonly Regex linePattern = new(@"^(?:\s*([a-zA-Z0-9_]+):)?\s*(?:(?:(?:(?:(global|local|const)\s+)?([a-zA-Z0-9_]+)\s*=|(?:([+\-&!*?$%=~}]+)\s*)?([a-zA-Z0-9]+))\s*(.*?)\s*({)?)|(}))?\s*(;.+)?$", RegexOptions.Compiled);
        private static readonly Regex valueRegex = new(@"^\s*(?:(-?[1-9][0-9]*\b|0\b)|(0x[0-9a-fA-F]+\b)|(0[0-7]+\b)|([01]+b\b)|([a-zA-Z$_][a-zA-Z0-9$_]*)|""([^""]*(?:""""[^""]*)*)""|(((?<open>\()[^()]*)+([^()]*(?<-open>\)))+(?(open)(?!))))", RegexOptions.Compiled);
        private static readonly Regex operatorRegex = new(@"^\s*(<<|>>|>=|<=|!=|is not|is|as|[+\-*/%|=\^<>&])\s*", RegexOptions.Compiled);
